    ...
    for (let {
      ...
      movieHtml = /*html*/ `<div class="movie-container ${ageGroup}"><div class="currentMovieIMG1">
                ...
                <div class="currentMovieTitleDiv">
                    ...
                    <article><button class="general-button article-button" onclick = "buttonPage('${title}')">Mer info</button><button class="general-button article-button" onclick='history.pushState(null, null, "#tickets/film=${title.replaceAll(" ", "-")}" );window.dispatchEvent(new HashChangeEvent("hashchange"));'>Köp biljett</button></article></article><hr>
                      <p>${description}
                      </p>
                  </div></div>`;
      i++;
      ...
      let movieHtml = /*html*/ `
                          <div>
                              ...
                              <div class="button-title">
                                <button class="general-button" onclick = 'history.pushState(null, null, "#currentMovies" );window.dispatchEvent(new HashChangeEvent("hashchange"));'>Gå tillbaka</button><button class="general-button" onclick='history.pushState(null, null, "#tickets/film=${title.replaceAll(" ", "-")}" );window.dispatchEvent(new HashChangeEvent("hashchange"));'>Köp biljett</button>
                                </div>
                              </div>
                                <hr width='100%'>
